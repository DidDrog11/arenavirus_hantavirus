...
if (length(args) < 5) {
  stop("Usage: Rscript run_cophylogeny_hanta.R <assoc_matrix_csv> <host_tree_nwk> <virus_tree_mcc> <ncbi_map_csv> <out_dir>")
}
...
# If labels are full headers, extract the species segment after splitting on "|"
ptree$tip.label <- sapply(ptree$tip.label, function(x) {
  if (grepl("\\|", x)) {
    parts <- trimws(strsplit(x, "\\|", fixed = FALSE)[[1]])
    hit <- parts[grepl("hantavirus|mobatvirus|thottimvirus", tolower(parts))]
    if (length(hit) > 0) {
      return(hit[1])
    }
  }
...
# If tips already look like species names, skip accession mapping
if (any(grepl("hantavirus|mobatvirus|thottimvirus", tolower(ptree$tip.label)))) {
  ptree$tip.label <- gsub("_", " ", ptree$tip.label)
} else {
  # fall back to accession mapping
...
write.csv(assoc, file = file.path(out_dir, "hanta_assoc_matrix_pruned.csv"))
write.tree(htree, file = file.path(out_dir, "hanta_host_tree_pruned.nwk"))
write.tree(ptree, file = file.path(out_dir, "hanta_virus_tree_pruned.nwk"))
...
jpeg(file.path(out_dir, "cophylo_hanta.jpeg"), width = 2500, height = 1600, res = 300)
...
# export residuals per host-parasite pair
