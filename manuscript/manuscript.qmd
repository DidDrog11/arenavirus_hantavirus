...
data <- read_rds(here("data", "database", "Project_ArHa_database_2025-08-25.rds"))
...
### Taxonomic Biases in Host and Pathogen Surveillance

To formally test the drivers of these observed biases, we fitted a series of nested Generalized Additive Models (GAMs) predicting the total number of pathogen samples collected for each host species. On our main dataset of 2,123 species with complete trait data, a model including host biological traits and geography explained 47.3% of the deviance in sampling effort. Both host geographic range size (GAM, edf=3.44, p<2e-16) and adult body mass (GAM, edf=5.88, p<2e-16) were highly significant, non-linear predictors. After accounting for these biological traits, the apparent sampling bias towards the Neotropic and Afrotropic realms was no longer statistically significant (p > 0.1), while persistent, strong positive effects remained for the Palearctic (p < 0.001) and Nearctic (p < 2e-05) realms.

In a focused sub-analysis on the 355 species with available synanthropy data, a species' propensity to live near humans was the most powerful predictor of sampling intensity. Species classified as Occasionally Synanthropic (p < 2e-14) or Totally Synanthropic (p < 2e-05) were associated with a dramatic and highly significant increase in sampling effort. In this final, comprehensive model, the inclusion of synanthropy attenuated most other effects, with only the Nearctic and Indomalayan realms retaining a significant independent association with sampling effort.

Our analysis reveals a profound taxonomic bias in global surveillance efforts. Of the [e.g., 3,457] known extant species of rodents and shrews, only [e.g., 745 (21.5%)] were found to have been sampled for either arenaviruses or hantaviruses in our systematic review. The sampling effort is highly concentrated in a few families, such as Muridae and Cricetidae, while many other families remain completely unsampled (Fig. X, Circular family plots). Similarly, pathogen surveillance is concentrated on a small number of well-known zoonotic species, such as Sin Nombre orthohantavirus and Lassa mammarenavirus, while many other known viral species have been tested for far less frequently (Fig. Y, Pathogen sampling bar chart).

To formally test the drivers of these observed biases, we fitted a series of nested Generalized Additive Models (GAMs) predicting the total number of pathogen samples collected for each host species. On our primary analytic dataset of 2,123 species with complete trait data, a model including host biological traits and geography explained 47.3% of the deviance in sampling effort. Both host geographic range size (GAM, edf=3.44, p<2e-16) and adult body mass (GAM, edf=5.88, p<2e-16) were highly significant, non-linear predictors. After accounting for these biological traits, the apparent sampling bias towards the Neotropic and Afrotropic realms was no longer statistically significant (p > 0.1), while persistent, strong positive effects remained for the Palearctic (p < 0.001) and Nearctic (p < 2e-05) realms.

In a focused sub-analysis on a secondary analytic dataset (the 355 species with available synanthropy data), a species' propensity to live near humans was the most powerful predictor of sampling intensity. Species classified as Occasionally Synanthropic (p < 2e-14) or Totally Synanthropic (p < 2e-05) were associated with a substantial increase in sampling effort. In this final, comprehensive model, the inclusion of synanthropy attenuated most other effects, with only the Nearctic and Indomalayan realms retaining a significant independent association with sampling effort.

### Gaps in Genetic Data Representativeness

Beyond simple sampling, we found significant gaps between the number of animals sampled and the availability of public genetic data. Geographically, surveillance is heavily concentrated in a few key regions, with [e.g., North America and Europe] showing a high proportion of sampled hosts having associated sequences. In contrast, vast regions in [e.g., Central Africa and Southeast Asia], despite having documented sampling efforts, are "genetic cold spots" with a very low proportion of sequenced samples (Fig. Z, Bivariate map). This disparity is also evident at the taxonomic level. Certain well-studied host species, such as [e.g., Peromyscus maniculatus or Myodes glareolus], have a high proportion of their sampled populations represented by genetic data. However, many other heavily sampled species, including [e.g., Mastomys natalensis], have a disproportionately low number of associated sequences relative to the thousands of individuals that have been sampled (Fig. W, Taxonomic representativeness plot).

## Geographic and Temporal Gaps

[Placeholder for Results] Presentation of maps and statistical results highlighting major geographic regions where known hosts exist but sampling is absent. Results of the correlation between sampling effort and socio-economic factors.

[Placeholder for Results] Presentation of plots showing trends in sampling effort and detected diversity over time, highlighting periods of intensified research.

## Host-Pathogen Network Structures

[Placeholder for Results] Visualization and description of the Arenaviridae and Hantaviridae networks.

[Placeholder for Results] Results of the quantitative comparison of the two network structures.

[Placeholder for Results] Identification of keystone host and pathogen taxa.
...
Conclusion: This work provides both a valuable, novel data resource and a quantitative framework for understanding the landscape of zoonotic virus surveillance. By identifying
